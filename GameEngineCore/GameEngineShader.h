...
protected:
	...
	//이름이 다른 리소스세터 여러개로 같은 리소스를 한 셰이더에서 중복해서 사용하는것은 전혀 문제없다.
	...
	//이렇게 리소스세터들을 동적할당해서 셰이더리소스들을 보관한다면, 
	// 다형성을 이용해서 ShaderResSetter를 상속받은 모든 셰이더리소스들을 하나의 컨테이너로 더 적은 메모리로 관리할 수 있다.
	// 하지만 그정도로 최적화된 메모리관리가 아직은 필요 없고, 
	// 리소스세터를 동적할당해서 포인터만 보관하면 같은 리소스 세터들의 리소스까지 전부 통일되므로 위와같이 저장한다.
