...
public:
	...
	float4 GetMouseWorldPosition();		//마우스 포인터의 위치를 월드좌표(화면 정중앙이 0, 0)로 받아오는 함수.
	float4 GetMouseWorldPositionToActor();
	void SetCameraOrder(CameraOrder _order);	//할 일이 있을지는 모르겠지만, 카메라 오더 중간변경 금지.
...
private:
	...
	//뷰행렬: 월드공간에 배치된 여러 오브젝트들을, 
	// (0, 0, 0) 위치에 있는 카메라의 시야범위(뷰 스페이스. 보통 절두체(frustum) 형태를 가지기 때문에 뷰 절두체, 뷰 프러스텀이라고 한다)
	// 안에 어떻게 재배치해서 월드공간에 배치되었던 대로 화면 안에 보이게 할 지 계산하는데 필요한 행렬. 
	//월드 전체의 오브젝트를 재배치하는 특성상, 뷰행렬은 월드 전체에 적용된다.
	//카메라가 뷰행렬만큼 이동 회전했다고 생각하지만 실제로는 카메라는 가만히 있고,
	// 오브젝트들이 뷰행렬의 반대로 같은 값만큼 움직인 것이다.
	// 그래서 실제 뷰행렬로 사용되는건 구한 행렬의 전치행렬이다.

	float4x4 projectionMatrix_;
	//투영행렬: 뷰행렬로 재배치된 뷰스페이스 안의 오브젝트들을 2차원으로 변환하는 과정을 투영변환이라고 하는데, 
	// 이 과정에 필요한 행렬. 크게 직교투영과 원근투영 두가지 투영방식으로 나눈다. 
	//투영행렬을 거치고 난 모든 뷰스페이스 안의 정점들의 좌표는 -1 <= x <= 1, -1 <= y <= 1, 0 <= z <=1 범위로 들어오게 된다.
	//뷰스페이스 밖에 있어서 저 값을 벗어난 정점들은 투영 대상에서 제외되어 화면에 그려지지 않는다.
