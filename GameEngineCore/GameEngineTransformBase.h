...
private:
	...
	//기본적으로 캐시의 지역성 때문에 데이타들은 물리적으로 가깝게 배치할수록 효율이 좋아지지만, 
	// 일반적인 멤버변수들은 숫자가 적고 한개의 크기가 크며 런타임 이전에 메모리 수요를 예측하기 어렵기 때문에 
	// 값형으로 저장해서 물리적으로 데이타를 가깝게 배치해서 캐시 적중률을 끌어올렸을때 생기는 성능 상승보다, 
	...
	// 동적할당을 해서 데이타가 흩어졌을때 생기는 성능 저하가 다른 멤버변수들에 비해 더 크고,
