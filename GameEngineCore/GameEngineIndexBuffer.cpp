...
)
{
	...
	newRes->CreateIndexBuffer(_initializationData, _indexSize, _indexCount);
	return newRes;
}
...
void GameEngineIndexBuffer::CreateIndexBuffer(const void* _initializationData, UINT _indexSize, UINT _indexCount)
{
	indexSize_ = _indexSize;
	indexCount_ = _indexCount;

	resData_.pSysMem = _initializationData;	//버퍼 초기데이터 설정.
