...
void GameEngineIndexBuffer::Setting()
{
	...
	GameEngineDevice::GetContext()->IASetIndexBuffer(//인풋 어셈블러 과정에서 인덱스버퍼를 파이프라인에 연결하는 함수.
		...
		DXGI_FORMAT::DXGI_FORMAT_R32_UINT,		//인덱스버퍼의 데이타 형식. 
...
void GameEngineIndexBuffer::CreateIndexBuffer(const void* _data, UINT _indexSize, UINT _indexCount)
{
	...
	bufferDesc_.ByteWidth = indexSize_ * indexCount_;	//버퍼 구조체의 바이트크기 등록.
	...
	bufferDesc_.BindFlags = D3D11_BIND_INDEX_BUFFER;	//이 버퍼를 파이프라인에 어떤 용도로 사용하는가.
